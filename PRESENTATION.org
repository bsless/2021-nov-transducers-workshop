...
And with transducers, we can also do this:

#+begin_src clojure
(->> xs (map f) (filter g) (partition-all 3) (map h) (filter p))
;; =>
(def xf (comp (map f) (filter g) (partition-all 3) (map h) (filter p)))
#+end_src

Where we couldn't compose other sequence functions the same way.
